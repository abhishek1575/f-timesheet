...
//-------------------------------- new code with condition of submit only on Monday and Tuesday--------------------------
...
//---------------------------------new code with dropdown options --------------------------
...
export default function CreateTimesheet({ onCancel, onTimesheetCreated }) {
  ...
  const [timesheet, setTimesheet] = useState({
    ...
    effort: "",
    comments: "",
    otherActivity: "", // New field for "Other" activity specification
  });
  ...
  const validate = () => {
    ...
    // Updated validation for projectName
  ...
  const saveDraft = async () => {
    ...
    try {
      ...
      const payload = {
        ...timesheet,
        projectName, // Use the custom text if "Other" is selected
        effort: Number(timesheet.effort),
      };
  ...
  const handleSubmit = async () => {
    // Check if today is Monday or Tuesday (commented out)
    // if (!canSubmitTimesheet()) {
    //   showSnackbar(
    //     "Timesheets can only be submitted on Mondays and Tuesdays",
    //     "error"
